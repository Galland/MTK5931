...
#endif
static void aml_gpio_input_mtk(int gpio);
static void aml_gpio_enable_mtk(int gpio);
static void aml_gpio_init_configure_mtk(unsigned gpio, bool get_status);
...
volatile static int bgf_INT_balance=0;
    ...
    )
{
    switch(state)
    {
        case PIN_STA_INIT:
            /*set to gpio output low, disable pull*/
            //aml_set_reg32_mask(P_PAD_PULL_UP_REG2,0x1<<18);
