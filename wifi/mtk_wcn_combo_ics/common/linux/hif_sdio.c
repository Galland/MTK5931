...
#include "../../../../customer/include/linux/mtk_wcn_gpio.h"
/*******************************************************************************
*                              C O N S T A N T S
********************************************************************************
*/
...
static void hif_sdio_irq_F2 (
    struct sdio_func *func
    );
static int mtk_sdio_irq_thread(void *_func);
#endif
...
#ifdef CONFIG_MTK_OOB_ONLY
struct task_struct	*mtk_sdio_thread = NULL;
volatile int mt6620_wifi_ext_int = 0;
struct sdio_func *hif_host = NULL; 
#endif
/*******************************************************************************
*                            P U B L I C   D A T A
********************************************************************************
...
struct sdio_func* hif_sdio_ctx_to_func_wapper ( 
    MTK_WCN_HIF_SDIO_CLTCTX ctx)
{    
    return hif_sdio_ctx_to_func(ctx);
}
...
{
    HIF_SDIO_DBG_FUNC("mt6620 wifi\n");
}
extern irqreturn_t mtk_oob_irq_handler(int i,void * arg)
{
 	struct sdio_func *sdhci_host = hif_host;
    			
	if(sdhci_host == NULL)
	{
		HIF_SDIO_INFO_FUNC("%s:Out of band GPIO interrupt fired way too early\n", __func__);
 	}
	else{
		// Let sdio_irq_thread handle the interrupt
		if(sdhci_host->card->host->sdio_irq_thread){
            disable_irq_nosync(mtk_oob_irq);
            mt6620_wifi_ext_int = 1;
         //  wake_up_process(mtk_sdio_thread);
          wake_up_process(sdhci_host->card->host->sdio_irq_thread);
		}
		else
			HIF_SDIO_INFO_FUNC("%s:host->sdio_irq_thread(NULL)\n",__func__);			
	}		
	return IRQ_HANDLED;
}
EXPORT_SYMBOL(mtk_oob_irq_handler);
static int mtk_sdio_irq_thread(void *_func)
 {
     struct sdio_func *func = _func;
    ...
    do {
        ...
        set_current_state(TASK_INTERRUPTIBLE);
            if(mt6620_wifi_ext_int){
               mt6620_wifi_ext_int = 0;
               enable_irq(mtk_oob_irq); 
            }
...
wifi_on_exist:
    ...
    if ( (clt_index = g_hif_sdio_probed_func_list[probe_index].clt_idx) >= 0 )    /* the function has been registered */
    {
        ...
        //4.1 enable this function
#ifdef CONFIG_MTK_OOB_ONLY
        //levi add by 2012-11-12
        hif_host = func;
#endif
        //end
        ...
        sdio_claim_host(func);
#ifdef CONFIG_MTK_OOB_ONLY
        ret = sdio_claim_irq(func, hif_sdio_irq_F2);
#else   
        ret = sdio_claim_irq(func, hif_sdio_irq);
#endif
        sdio_release_host(func);
...
#ifdef CONFIG_MTK_OOB_ONLY
		//mtk_sdio_thread =kthread_run(mtk_sdio_irq_thread, func, "mtksdioirqd/%s",card_hostname(func->card->host));
        kthread_stop(func->card->host->sdio_irq_thread);
		func->card->host->sdio_irq_thread =kthread_run(mtk_sdio_irq_thread, func, "mtksdioirqd/%s",card_hostname(func->card->host));
 #endif
...
EXPORT_SYMBOL(hif_sdio_ctx_to_func_wapper);
