...
{
#define LPS_LEAVE_TIMEOUT_MS 1000

	struct mlme_priv	*pmlmepriv = &(Adapter->mlmepriv);
	u32 wait_lps_leave_start_time;
...
_func_enter_;
	...
	{ //connect
		...
		//DBG_8192C("==> leave LPS.......\n");
		LPS_Leave(Adapter);
		wait_lps_leave_start_time = rtw_get_current_time();
		if (Adapter->pwrctrlpriv.bLeisurePs)
		{
			...
			{
				rtw_usleep_os(100);
				if(rtw_get_passing_time_ms(wait_lps_leave_start_time)>LPS_LEAVE_TIMEOUT_MS)
				{
					DBG_8192C("Wait for FW LPS leave more than %u ms!!!\n", LPS_LEAVE_TIMEOUT_MS);
					break;
				}
				Adapter->HalFunc.GetHwRegHandler(Adapter, HW_VAR_FWLPS_RF_ON, (u8 *)(&bAwake));
			}
