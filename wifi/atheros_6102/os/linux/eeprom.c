...
/* ATHENV */
#ifdef ANDROID_ENV
#ifndef CONFIG_AR6102_BOARD_DATA_FILE
...
static int read_eeprom_from_file(char *fake_file, A_UCHAR *eeprom_data)
{
	...
	if(request_firmware(&fw_entry, fake_file, &dev) == 0)
	{
		memcpy(eeprom_data,fw_entry->data,fw_entry->size);
		release_firmware(fw_entry);
		return 0;
	}
	printk("%s: get file %s error\n", __FUNCTION__, fake_file);
	return -1;
}
#endif
