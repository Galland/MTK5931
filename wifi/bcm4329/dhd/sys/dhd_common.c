...
dhd_common_init(void)
{
	 ...
	 */	 
	dhd_msg_level = DHD_ERROR_VAL; //mcli DHD_TRACE_VAL|DHD_INFO_VAL|DHD_CTL_VAL|DHD_EVENT_VAL|
#ifdef CONFIG_SAMSUNG_BCM4329_FW_PATH
	strncpy(fw_path, CONFIG_SAMSUNG_BCM4329_FW_PATH, MOD_PARAM_PATHLEN-1);
#else
	fw_path[0] = '\0';
#endif
#ifdef CONFIG_SAMSUNG_BCM4329_NVRAM_PATH
	strncpy(nv_path, CONFIG_SAMSUNG_BCM4329_NVRAM_PATH, MOD_PARAM_PATHLEN-1);
#else
	nv_path[0] = '\0';
#endif
    printk(KERN_ERR "fw_path:%s\n",fw_path);
    printk(KERN_ERR "nv_path:%s\n",nv_path);
}
