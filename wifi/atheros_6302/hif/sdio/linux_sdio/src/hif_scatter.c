...
#else
		blocks = pReq->TotalLength / HIF_MBOX_BLOCK_SIZE;
		status = sdio_io_rw_extended(device->func->card,rw,device->func->num,pReq->Address,opcode,busrequest->buffer,blocks,HIF_MBOX_BLOCK_SIZE);
#endif
